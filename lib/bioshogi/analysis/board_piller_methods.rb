  ...
  module Analysis
    module BoardPillerMethods
      ...
      def place_on(soldier, options = {})
        ...
        if c > Dimension::DimensionRow.dimension_size
        ...
        self.piece_piller_by_latest_piece = (c == Dimension::DimensionRow.dimension_size) # 最後の駒が反映される
      end

      # 現在の状態は駒柱がある状態か？
      def piece_piller_by_latest_piece?
        piller_counts.each_value.any? { |c| c >= Dimension::DimensionRow.dimension_size } # O(n) になるので使いたくない
      end
      ...
      def safe_delete_on(*)
        super.tap do |soldier|
          if soldier
            ...
            piller_counts[soldier.place.x.value] = c
            self.piece_piller_by_latest_piece = (c == Dimension::DimensionRow.dimension_size)
          end
        end
      end
