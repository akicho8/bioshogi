...
# tp Analysis::TacticInfo.all_soldier_lookup_hash1.inject({}) {|a, (k, v)| a.merge(k.name => v.count) }
# tp Analysis::TacticInfo.all_soldier_lookup_hash2.inject({}) {|a, (k, v)| a.merge(k.name => v.count) }
tp Analysis::TacticInfo.primary_soldiers.inject({}) { |a, (k, v)| a.merge(k.name => v.count) }

tp Analysis::TacticInfo.all_soldier_lookup_hash.inject({}) { |a, (k, v)| a.merge(k.name => v.count) }
