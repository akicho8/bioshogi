...
RSpec.describe Bioshogi::Formatter::Animation::AnimationMp4Builder, animation: true do
  ...
  it "クロスフェイドあり" do
    info = Bioshogi::Parser.parse("position startpos moves 7g7f 3c3d 8h2b+ 8c8d 2b3a 8d8e 3a4a 8e8f 4a5a")
    bin = info.to_animation_mp4(page_duration: 1.0, end_duration: 2, width: 2, height: 2)
    filename = Pathname("_outout.zip")
    filename.write(bin)
    video, audio = Bioshogi::Media.streams(filename)
    pp audio if $0 == "-"
