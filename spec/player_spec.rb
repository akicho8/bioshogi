...
EOT
    ...
    describe "移動" do
      describe "できる" do
        # it "foo" do
        #   player = Mediator.player_test
        #   # puts player.mediator
        #   player.soldier_create("７七歩")
        #   puts player.mediator
        #   player.execute("７六歩")
        #   puts player.mediator
        # end
        ...
        # このテストは Mediator.read_spec で書くので不要
        # it "真下と斜め右にある銀の右側の銀が左上に移動し、KI2形式の指し手が「○右」となる" do
        #   Mediator.player_test(init: "▲29銀 ▲19銀", exec: "28銀(19)").runner.hand_log.to_kif_ki2.should== ["２八銀(19)", "２八銀右"]
        #
        #   # mediator = Mediator.test1(init: "▲29銀 ▲19銀", exec: ["28銀(19)"])
      ...
      describe "不成" do
        describe "できる" do
          ...
          describe "金が不成するケース" do
            ...
            it "不成の指定をしなかった" do
              Mediator.player_test(init: "１四金", exec: "１三金").runner.hand_log.to_kif_ki2.should== ["１三金(14)", "１三金"]
            end
          end
        end
        describe "できない" do
    ...
    describe "打つ" do
      ...
      describe "打の意味が重要なケース" do
        it "打を明示していないので駒を動かした" do
          Mediator.player_test(pieces_add: "飛", init: "１一飛", exec: "１二飛").runner.hand_log.to_kif_ki2_csa.should == ["１二飛(11)", "１二飛不成", "+1112HI"]
        end
        it "打を明示したので駒を打つ" do
          Mediator.player_test(pieces_add: "飛", init: "１一飛", exec: "１二飛打").runner.hand_log.to_kif_ki2_csa.should == ["１二飛打", "１二飛打", "+0012HI"]
          Mediator.player_test(pieces_add: "飛", init: "１一飛", exec: "１二飛合").runner.hand_log.to_kif_ki2_csa.should == ["１二飛打", "１二飛打", "+0012HI"]
        end
      end
    end
...
EOT
    ...
    #   player1 = Mediator.player_test(init: "５九玉", exec: "５八玉")
