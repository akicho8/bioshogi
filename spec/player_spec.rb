    ...
    describe "移動" do
      ...
      describe "できない" do
        ...
        it "推測結果が複数パターンがあったので" do
          expect { Mediator.player_test_soldier_names(init: ["６九金", "４九金"], execute: "５八金") }.to raise_error(AmbiguousFormatError, /５八に移動できる駒が2つありますが表記が曖昧なため特定できません。移動元は「６九の金」か「４九の金」のどっちかな？/)
          expect { Mediator.player_test_soldier_names(init: ["６九金", "４九金"], execute: "５八金上") }.to raise_error(AmbiguousFormatError, /５八に移動できる駒が2つありますが「上」からは特定できません。移動元は「６九の金」か「４九の金」のどっちかな？/)
        end
