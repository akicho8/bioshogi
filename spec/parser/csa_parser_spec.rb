...
module Bioshogi
  describe Parser::CsaParser do
    ...
    it "棋譜部分のパース" do
      assert { Parser::CsaParser.parse("1234FU").pi.move_infos.first[:input] == "1234FU" }
      assert { Parser::CsaParser.parse("+1234FU").pi.move_infos.first[:input] == "+1234FU" }
      assert { Parser::CsaParser.parse("+1234FU,T1").pi.move_infos.first == {input: "+1234FU", used_seconds: 1} }
    end
