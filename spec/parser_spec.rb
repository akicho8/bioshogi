...
module Bioshogi
  RSpec.describe Parser do
    describe "判定" do
      ...
      it "空" do
        assert { Parser.parse("") rescue $!.class == Bioshogi::FileFormatError }
        assert { Parser.parse(nil) rescue $!.class == Bioshogi::FileFormatError }
      end
    end

    it "ヘッダー行のセパレータは全角セミコロン" do
      assert { Parser.parse("a：b").pi.header.to_h == {"a" => "b"} }
      assert { Parser.parse("a:b：c").pi.header.to_h == {"a:b" => "c"} }
    end

    it "ヘッダー行のセパレータに半角を含めると時間の部分のセミコロンと衝突するので対応しない" do
      assert { Parser.parse("a:b") rescue $!.class == Bioshogi::FileFormatError }
    end

    it "日時の場合正規化する" do
      assert { Parser.parse("開始日時：2000-1-1  1:23:45").pi.header.to_h == {"開始日時" => "2000/01/01 01:23:45"} }
      assert { Parser.parse("終了日時：2000/1/1 01:23:45").pi.header.to_h == {"終了日時" => "2000/01/01 01:23:45"} }
    end
  end
end
...
# >>      Failure/Error: raise FileFormatError, "棋譜のフォーマットが不明です : #{source}"
# >> 
# >>      Bioshogi::FileFormatError:
# >>        棋譜のフォーマットが不明です : a:b
# >>      # ./lib/bioshogi/parser.rb:24:in `parse'
# >>      # -:25:in `block (2 levels) in <module:Bioshogi>'
# >> 
...
# >>        棋譜のフォーマットが不明です : 
# >>      # ./lib/bioshogi/parser.rb:24:in `parse'
# >>      # -:14:in `block (4 levels) in <module:Bioshogi>'
# >>      # <internal:prelude>:137:in `__enable'
# >>      # <internal:prelude>:137:in `enable'
# >>      # <internal:prelude>:137:in `__enable'
# >>      # <internal:prelude>:137:in `enable'
# >>      # -:14:in `block (3 levels) in <module:Bioshogi>'
# >> 
# >> Top 5 slowest examples (0.01695 seconds, 76.3% of total time):
...
# >>     0.00064 seconds -:24
# >> 
# >> Finished in 0.02222 seconds (files took 1.48 seconds to load)
