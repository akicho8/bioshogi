...
module Bioshogi
  describe "usi" do
    it "出力" do
      ...
      assert { mediator.to_history_sfen == "position startpos" }
      ...
      assert { mediator.to_history_sfen == "position startpos moves 7i6h" }
      mediator.execute("△２四銀打")
      assert { mediator.hand_logs.last.to_sfen == "S*2d" }
      assert { mediator.to_history_sfen == "position startpos moves 7i6h S*2d" }
      assert { mediator.initial_state_board_sfen == "position startpos" }
      # puts mediator.board
      assert { mediator.to_history_sfen == "position startpos moves 7i6h S*2d" }
    end
    ...
    it "入力" do
      ...
      assert { usi.mediator.to_history_sfen == "position startpos moves 7i6h" }
      usi.execute("position startpos")
      assert { usi.mediator.to_history_sfen ==  "position startpos" }
