...
module Bushido
  describe Soldier do
    ...
    describe "#moveable_points2" do
      it "初期配置での移動可能な座標" do
        player.piece_plot
        board["7七"].moveable_points2.collect(&:name).should == ["7六"]                                    # 歩
        board["9九"].moveable_points2.collect(&:name).should == ["9八"]                                    # 香
        board["8九"].moveable_points2.collect(&:name).should == []                                         # 桂
        board["7九"].moveable_points2.collect(&:name).should == ["7八", "6八"]                             # 銀
        board["6九"].moveable_points2.collect(&:name).should == ["7八", "6八", "5八"]                      # 金
        board["5九"].moveable_points2.collect(&:name).should == ["6八", "5八", "4八"]                      # 玉
        board["8八"].moveable_points2.collect(&:name).should == []                                         # 角
        board["2八"].moveable_points2.collect(&:name).should == ["3八", "4八", "5八", "6八", "7八", "1八"] # 飛
      end
