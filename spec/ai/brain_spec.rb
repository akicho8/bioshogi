...
RSpec.describe Bioshogi::AI::Brain do
  it "初手が嬉野流になるよう誘導される" do
    ...
    brain = container.player_at(:black).brain(diver_class: Bioshogi::AI::Diver::NegaAlphaDiver, evaluator_class: Bioshogi::AI::Evaluator::Level3)
    @records = brain.iterative_deepening(depth_max_range: 0..0)
    assert { @records.first[:hand].to_s == "▲６八銀(79)" }
  end

  it "works" do
    Bioshogi::Dimension.change([3, 3]) do
      container = Bioshogi::Container::Simple.new
  ...
  it "works" do
    [Bioshogi::AI::Diver::NegaAlphaDiver, Bioshogi::AI::Diver::NegaScoutDiver].each do |diver_class|
      Bioshogi::Dimension::Row.promotable_disabled do
        Bioshogi::Dimension.change([2, 5]) do
          container = Bioshogi::Container::Simple.new
          container.board.placement_from_shape <<~EOT
          +------+
            | ・v香|
  ...
  describe "自動的に打つ" do
    it "盤上の駒を動かす" do
      container = Bioshogi::Container::Simple.start
  ...
  it "盤上の駒の全手筋" do
    Bioshogi::Dimension.change([1, 5]) do
      container = Bioshogi::Container::Simple.facade(init: "▲１五香")
