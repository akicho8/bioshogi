...
bin = info.to_mp4(one_frame_duration: 1, tmpdir_remove: false, acrossfade_duration: 0, main_volume: 0.75, end_frames: 5)
Pathname("_output.mp4").write(bin) # => 174241
Media.duration("_output.mp4")      # => 6.525
Media.p("_output.mp4")
tp Media.format("_output.mp4")
`open -a 'google chrome' _output.mp4`
# >> cd /var/folders/9c/_62dfc8502g_d5r05zyfwlxh0000gn/T/d20210817-13329-1bnmphe
# >> move: 1 / 12
# >> move: 11 / 12
# >> frame_count: 13
...
# >> total_d: 6.5
# >> switch_turn: 5
# >> run: ffmpeg -v warning -hide_banner -r 1000/500 -y -i _output0.mp4 -c:v libx264 -pix_fmt yuv420p -movflags +faststart -y _output1.mp4
# >> elapsed: 2s
# >> run: ffmpeg -v warning -stream_loop -1 -i /Users/ikeda/src/bioshogi/lib/bioshogi/assets/audios/loop_bgm1.m4a -t 3.5 -c copy -y _part1.m4a
# >> elapsed: 1s
# >> _part1.m4a: 3.505
# >> run: ffmpeg -v warning -stream_loop -1 -i /Users/ikeda/src/bioshogi/lib/bioshogi/assets/audios/loop_bgm2.m4a -t 4.0 -c copy -y _part2.m4a
# >> elapsed: 1s
# >> _part2.m4a: 4.016
# >> run: ffmpeg -v warning -i _part1.m4a -i _part2.m4a -filter_complex "acrossfade=duration=1.0" _concat.m4a
# >> elapsed: 1s
# >> run: ffmpeg -v warning -i _concat.m4a -af "afade=t=out:start_time=3.5:duration=3.0" _same_lengh.m4a
# >> elapsed: 1s
# >> _same_lengh.m4a: 6.525
# >> run: ffmpeg -v warning -i _output1.mp4 -i _same_lengh.m4a -c copy -y _output2.mp4
# >> elapsed: 1s
# >>   Stream #0:0(und): Video: h264 (High) (avc1 / 0x31637661), yuv420p, 1200x630, 81 kb/s, 2 fps, 2 tbr, 16384 tbn, 4 tbc (default)
...
# >> |         duration | 0:00:06.525000                                                                                                       |
# >> |             size | 170.157227 Kibyte                                                                                                    |
# >> |         bit_rate | 213.628000 Kbit/s                                                                                                    |
# >> |      probe_score | 100                                                                                                                  |
