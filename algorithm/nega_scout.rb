require "./nega_max"

class NegaScout < NegaMax
  def compute_score(*args)
    nega_scout(*args)
  end

  def nega_scout(turn:, depth_max:, depth: 0, alpha: -Float::INFINITY, beta: Float::INFINITY)
    perform_out_of_time_check

    player = mediator.player_at(turn)
    children = mediator.available_points(player)

    # 一番深い局面に達したらはじめて評価する
    if depth_max <= depth
      return [mediator.evaluate(player), []] # 現局面手番視点
    end

    # 合法手がない場合はパスして相手に手番を渡す
    if children.empty?
      v, way = nega_scout(turn: turn + 1, depth_max: depth_max, depth: depth + 1, alpha: -beta, beta: -alpha)
      v = -v
      return [v, [:pass, *way]]
    end

    # 再帰を簡潔に記述するため
    recursion = -> _point, alpha2, beta2 {
      mediator.put_on(player, _point) do
        v, way = nega_scout(turn: turn + 1, depth_max: depth_max, depth: depth + 1, alpha: alpha2, beta: beta2)
        v = -v
        [v, way]
      end
    }

    # 効果的なもの順に並び換える
    children = mediator.move_ordering(player, children)

    # 最善候補を通常の窓で探索
    point = children.shift
    v, way = recursion.(point, -beta, -alpha)
    max_v = v
    if beta <= v
      return [v, [point, *way]]
    end
    if alpha < v
      alpha = v
    end

    forecast = []
    children.each do |point|
      v, way = recursion.(point, -(alpha + 1), -alpha) # null window search
      if beta <= v
        return [v, [point, *way]]
      end
      if alpha < v
        alpha = v
        v, way = recursion.(point, -beta, -alpha) # 通常の窓で再探索
        if beta <= v
          return [v, [point, *way]]
        end
        if alpha < v
          alpha = v
        end
      end
      if max_v < v
        max_v = v
        forecast = [point, *way] # この way どこの way ?
      end
    end

    [max_v, forecast]
  end
end

if $0 == __FILE__
  NegaScout.new(dimension: 4, depth_max: 6).run             # => {:o=>2, :x=>14}
end
# >> ------------------------------------------------------------ [0] o 実行速度:0.379908
# >> ・○・・
# >> ・○○・
# >> ・○×・
# >> ・・・・
# >> |------+--------+----------------------+--------+------+------|
# >> | 順位 | 候補手 | 読み筋               | 評価値 | 形勢 | 時間 |
# >> |------+--------+----------------------+--------+------+------|
# >> |    1 | [1, 0] |                      |     -1 |   -1 |      |
# >> |    2 | [0, 1] |                      |     -1 |   -1 |      |
# >> |    3 | [3, 2] | [3, 3] [2, 3] [3, 1] |     -1 |   -1 |      |
# >> |    4 | [2, 3] | [3, 3] [3, 2] [3, 1] |     -1 |   -1 |      |
# >> |------+--------+----------------------+--------+------+------|
# >> ------------------------------------------------------------ [1] x 実行速度:0.259882
# >> ×○・・
# >> ・×○・
# >> ・○×・
# >> ・・・・
# >> |------+--------+-------------------------------------------+--------+------+------|
# >> | 順位 | 候補手 | 読み筋                                    | 評価値 | 形勢 | 時間 |
# >> |------+--------+-------------------------------------------+--------+------+------|
# >> |    1 | [0, 0] |                                           |      6 |   -6 |      |
# >> |    2 | [0, 2] | [2, 3] [3, 2] [3, 3] [2, 0] [1, 3] [0, 0] |     -2 |    2 |      |
# >> |    3 | [2, 0] |                                           |     -7 |    7 |      |
# >> |------+--------+-------------------------------------------+--------+------+------|
# >> ------------------------------------------------------------ [2] o 実行速度:0.104885
# >> ×○・・
# >> ○○○・
# >> ・○×・
# >> ・・・・
# >> |------+--------+--------+--------+------+------|
# >> | 順位 | 候補手 | 読み筋 | 評価値 | 形勢 | 時間 |
# >> |------+--------+--------+--------+------+------|
# >> |    1 | [0, 1] |        |     -3 |   -3 |      |
# >> |    2 | [3, 2] |        |     -9 |   -9 |      |
# >> |    3 | [2, 3] |        |     -9 |   -9 |      |
# >> |------+--------+--------+--------+------+------|
# >> ------------------------------------------------------------ [3] x 実行速度:0.076503
# >> ×××・
# >> ○○×・
# >> ・○×・
# >> ・・・・
# >> |------+--------+--------+--------+------+------|
# >> | 順位 | 候補手 | 読み筋 | 評価値 | 形勢 | 時間 |
# >> |------+--------+--------+--------+------+------|
# >> |    1 | [2, 0] |        |      8 |   -8 |      |
# >> |    2 | [0, 2] |        |      8 |   -8 |      |
# >> |------+--------+--------+--------+------+------|
# >> ------------------------------------------------------------ [4] o 実行速度:0.131499
# >> ×××・
# >> ○○○○
# >> ・○×・
# >> ・・・・
# >> |------+--------+--------+--------+------+------|
# >> | 順位 | 候補手 | 読み筋 | 評価値 | 形勢 | 時間 |
# >> |------+--------+--------+--------+------+------|
# >> |    1 | [3, 1] |        |     -6 |   -6 |      |
# >> |    2 | [3, 0] |        |     -8 |   -8 |      |
# >> |    3 | [3, 3] |        |     -8 |   -8 |      |
# >> |    4 | [3, 2] |        |     -9 |   -9 |      |
# >> |------+--------+--------+--------+------+------|
# >> ------------------------------------------------------------ [5] x 実行速度:0.061923
# >> ×××・
# >> ××○○
# >> ×××・
# >> ・・・・
# >> |------+--------+----------------------+--------+------+------|
# >> | 順位 | 候補手 | 読み筋               | 評価値 | 形勢 | 時間 |
# >> |------+--------+----------------------+--------+------+------|
# >> |    1 | [0, 2] |                      |      9 |   -9 |      |
# >> |    2 | [1, 3] | [0, 3] [0, 2]        |      9 |   -9 |      |
# >> |    3 | [3, 2] | [3, 0] [1, 3] [3, 3] |      1 |   -1 |      |
# >> |------+--------+----------------------+--------+------+------|
# >> ------------------------------------------------------------ [6] o 実行速度:0.03699
# >> ×××・
# >> ××○○
# >> ×○×・
# >> ○・・・
# >> |------+--------+---------------------------+--------+------+------|
# >> | 順位 | 候補手 | 読み筋                    | 評価値 | 形勢 | 時間 |
# >> |------+--------+---------------------------+--------+------+------|
# >> |    1 | [0, 3] | [1, 3] [2, 3] [3, 0] PASS |     -9 |   -9 |      |
# >> |    2 | [2, 3] | [3, 0] PASS               |    -11 |  -11 |      |
# >> |    3 | [1, 3] | [3, 0] PASS [3, 3] PASS   |    -12 |  -12 |      |
# >> |------+--------+---------------------------+--------+------+------|
# >> ------------------------------------------------------------ [7] x 実行速度:0.014484
# >> ×××・
# >> ××○○
# >> ×××・
# >> ○・×・
# >> |------+--------+--------+--------+------+------|
# >> | 順位 | 候補手 | 読み筋 | 評価値 | 形勢 | 時間 |
# >> |------+--------+--------+--------+------+------|
# >> |    1 | [2, 3] |        |     12 |  -12 |      |
# >> |    2 | [1, 3] |        |     10 |  -10 |      |
# >> |    3 | [3, 2] |        |     -2 |    2 |      |
# >> |------+--------+--------+--------+------+------|
# >> ------------------------------------------------------------ [8] o 実行速度:0.005789
# >> ×××・
# >> ××○○
# >> ××○・
# >> ○○×・
# >> |------+--------+-------------+--------+------+------|
# >> | 順位 | 候補手 | 読み筋      | 評価値 | 形勢 | 時間 |
# >> |------+--------+-------------+--------+------+------|
# >> |    1 | [1, 3] | [3, 3] PASS |    -12 |  -12 |      |
# >> |------+--------+-------------+--------+------+------|
# >> ------------------------------------------------------------ [9] x 実行速度:0.003319
# >> ×××・
# >> ××○○
# >> ×××・
# >> ○○××
# >> |------+--------+--------+--------+------+------|
# >> | 順位 | 候補手 | 読み筋 | 評価値 | 形勢 | 時間 |
# >> |------+--------+--------+--------+------+------|
# >> |    1 | [3, 3] | PASS   |     12 |  -12 |      |
# >> |    2 | [3, 0] |        |      8 |   -8 |      |
# >> |    3 | [3, 2] |        |     -2 |    2 |      |
# >> |------+--------+--------+--------+------+------|
# >> ------------------------------------------------------------ [10] o 実行速度:0.00021
# >> ×××・
# >> ××○○
# >> ×××・
# >> ○○××
# >> (pass)
# >> ------------------------------------------------------------ [11] x 実行速度:0.001354
# >> ××××
# >> ×××○
# >> ×××・
# >> ○○××
# >> |------+--------+--------+--------+------+------|
# >> | 順位 | 候補手 | 読み筋 | 評価値 | 形勢 | 時間 |
# >> |------+--------+--------+--------+------+------|
# >> |    1 | [3, 0] | PASS   |     12 |  -12 |      |
# >> |    2 | [3, 2] |        |      4 |   -4 |      |
# >> |------+--------+--------+--------+------+------|
# >> ------------------------------------------------------------ [12] o 実行速度:0.000154
# >> ××××
# >> ×××○
# >> ×××・
# >> ○○××
# >> (pass)
# >> ------------------------------------------------------------ [13] x 実行速度:0.000487
# >> ××××
# >> ××××
# >> ××××
# >> ○○××
# >> |------+--------+-------------------------------+--------+------+------|
# >> | 順位 | 候補手 | 読み筋                        | 評価値 | 形勢 | 時間 |
# >> |------+--------+-------------------------------+--------+------+------|
# >> |    1 | [3, 2] | PASS PASS PASS PASS PASS PASS |     12 |  -12 |      |
# >> |------+--------+-------------------------------+--------+------+------|
# >> |---+----|
# >> | o | 2  |
# >> | x | 14 |
# >> |---+----|
